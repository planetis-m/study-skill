...
```text
$skill-installer install the skill from repo planetis-m/study-skill with path study-assistant
```
...
Copy the `study-assistant/` folder into a scanned skills path, for example:

```bash
git clone https://github.com/planetis-m/study-skill.git ~/.agents/skills/study-skill
cp -R ~/.agents/skills/study-skill/study-assistant ~/.agents/skills/study-assistant
```
